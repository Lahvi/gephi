...
/**
 * The
 * <code>FilteredEdgesFactory</code> class contains the logic for creating
...
public class FilterEdgesFactory {
    ...
    public FilterEdgesFactory(Graph graph, Graph filteredGraph) {
        ...
        setFiltredEdges();
        directed = false;
        if (filteredGraph instanceof DirectedGraph) {
            directed = true;
        }
        ...
        while (!wrapperQueue.isEmpty()) {
            ...
            if (filteredGraph.contains(actualWrapper.lastNode)) {
                if (!filteredGraph.isAdjacent(sourceNode, actualWrapper.lastNode)) {
                    //nova hrana
                    filteredGraph.addEdge(edgesFactory.newEdge(sourceNode, actualWrapper.lastNode, actualWrapper.totalCost, directed));
                } else {
                    Edge existingEdge = filteredGraph.getEdge(sourceNode, actualWrapper.lastNode);
                    if (filtredGraphEdges.contains(existingEdge)) { //pokud je hrana v puvodnim vyfiltrovanem grafu tak ji nebudu nahrazovat drazsi cestou
    ...
    private void expandNodes(NodePathWrapper expandedNode) {
        ...
        for (int i = 0; i < neighboroughs.length; i++) {
            if (!visitedNodes.contains(neighboroughs[i])) {
                Edge edge = graph.getEdge(expandedNode.lastNode, neighboroughs[i]);
                if (edge != null) {
